...
object DataETL {
  ...
  val STOP_WORD_PATH: String = "C:\\Users\\only you\\Downloads\\spark-classify-master\\spark-classify-master\\src\\main\\resources\\stopWord.txt"
  ...
  def main(args: Array[String]): Unit = {
    ...
    fileNameList.foreach(fileName=> {
      ...
      count = count+1
      labels.put(count, fileName.replace(".txt", ""))//将count 代替名称变成label 保存
      ...
      while (line!=null) {
        ...
        if (!StringUtils.isBlank(line)){
          ...
          val splitWords: util.List[String] = AnsjUtil.participle(line)
