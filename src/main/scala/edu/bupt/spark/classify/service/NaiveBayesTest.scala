package edu.bupt.spark.classify.service

import edu.bupt.spark.classify.bean.Result
import com.alibaba.fastjson.JSON
import edu.bupt.spark.classify.service.DataETL.HDFS_PATH
import edu.bupt.spark.classify.utils.{AnsjUtil,FileUtil, HbaseUtil, HdfsUtil}
...
import edu.bupt.spark.classify.utils.AnsjUtil
...
object NaiveBayesTest {
  ...
  def batchTestModel(spark: SparkSession,hbaseUtil: HbaseUtil, path: String) = {
    val test: Dataset[Row] = spark.read.json(path)//从HDFS中获取数据
  ...
  def testModel(spark: SparkSession, str: String): Unit = {
    val data: util.List[Row] = util.Arrays.asList(RowFactory.create(AnsjUtil.participle(str)))
