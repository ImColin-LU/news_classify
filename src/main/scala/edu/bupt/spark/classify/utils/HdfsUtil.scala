...
import java.io.{File, FileInputStream, FileOutputStream, FileReader, IOException, InputStream, LineNumberReader, OutputStream}
...
class HdfsUtil {
  ...
  def appendBatchText(fileSystem: FileSystem,hdfsPath:Path,dataList:ListBuffer[String]): Unit ={
    ...
    dataList.foreach(data=>{
      apend.write(data.getBytes())
    })
    apend.close()
  }

  def appendText(fileSystem: FileSystem,hdfsPath:Path,data:String): Unit ={
    val apend: FSDataOutputStream = fileSystem.append(hdfsPath)
    apend.write(data.getBytes())
    apend.close()
  }
   ...
   * @return Array[String]
   */
  def getHDFSFilesName(fileSystem:FileSystem,hdfsDirectory:String): ListBuffer[String] ={
   ...
   * @return
   */
  def getHDFSFilesPath(fileSystem:FileSystem,hdfsDirectory:String):ListBuffer[Path]={
    val fsPath: Path = new Path(hdfsDirectory)
    val iterator = fileSystem.listFiles(fsPath, true)
    val list = new ListBuffer[Path]
    while (iterator.hasNext) {
      val pathStatus = iterator.next()
      val path: Path = pathStatus.getPath
      list.append(path)
    }
    list
  }
